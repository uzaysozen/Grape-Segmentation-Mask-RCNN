...
@app.route('/gallery')
def gallery():
  images = [name for name in os.listdir(output_images_directory) if os.path.isfile(os.path.join(output_images_directory, name))]
  return render_template('gallery.html',images=images, len=len(images))


@app.route('/predict', methods = ['POST'])
def predict():
  number_of_images = len([name for name in os.listdir(output_images_directory) if os.path.isfile(os.path.join(output_images_directory, name))])
  ...
  if request.method == 'POST':
    ...
    cv2.imwrite("static/output_images/Output"+str(number_of_images+1)+".png", cv2.cvtColor(image_np, cv2.COLOR_RGB2BGR))
